...
def test_default_timeouts() -> None:
    ...
    assert default_timeout.kill is None
...
def test_custom_timeouts() -> None:
    ...
    assert timeouts.kill == kill_timeout


def test_incorrect_run_timeout() -> None:
    for incorrect_value in [0, -1, float("nan"), float("inf")]:
        with pytest.raises(
            ValueError,
...
def test_incorrect_kill() -> None:
    for incorrect_value in [0, -1, float("nan"), float("inf")]:
        with pytest.raises(
            ValueError,
            match=r"kill timeout must be either None or > 0$",
        ):
            flnr.ExecutionTimeouts(kill=incorrect_value)
