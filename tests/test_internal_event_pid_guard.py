...
async def test_internal_pid_guard_trigger() -> None:
    ...
    # here we ensure that our monitor is never called, since process
...
async def test_internal_pid_guard_notrigger() -> None:
    ...
    # here we ensure that our monitor is actually called, despite the fact
    # that the underllying process is likely dead. The expectation is that
    # since there was not `.wait` from asyncio machinery, the process status
    # is not reaped by python.
